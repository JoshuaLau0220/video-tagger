...
class Config:
    def __init__(self, config_path="config.toml"):
        if not os.path.exists(config_path):
            self.jump_forward_interval = 10000
            self.jump_backward_interval = 10000
            ...
            with open(config_path, "r") as f:
                config = toml.load(f)
            self.jump_forward_interval = config["jump_forward_interval"] \
                if "jump_forward_interval" in config \
                else 10000
            self.jump_backward_interval = config["jump_backward_interval"] \
                if "jump_backward_interval" in config \
                else 10000
